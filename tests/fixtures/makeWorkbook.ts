 ...
 *
 * Contents: 3 students, 2 monthly sheets (يناير، فبراير), 2 employees with
...
export async function writeSampleWorkbook(): Promise<string> {
  ...
  // Monthly sheet builder ("#" in col B, student name in col C)
  const buildMonth = (name: string, rows: any[][]) => {
    const ws = wb.addWorksheet(name)
    setRow(ws, 1, 3, [`📅 ${name}`])
    setRow(ws, 3, 2, ['#', 'اسم الطالب', 'الخدمة', 'الوحدة', 'الكمية', 'السعر', 'الإجمالي', 'المدفوع', 'الرصيد', 'الحالة', 'ملاحظات'])
    rows.forEach((r, i) => setRow(ws, 4 + i, 2, [i + 1, ...r]))
  }

  // يناير: 3 students; one fully paid, one partial, one unpaid; plus a blank-name
