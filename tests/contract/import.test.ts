...
describe('importFromWorkbook — contract', () => {
  it('imports students, payments, employees, salaries and expenses (US1)', async () => {
    const summary = await importFromWorkbook(fixturePath)

    // Year resolved from IMPORT_DEFAULT_YEAR
    expect(summary.year).toBe(2025)

    // Students: 3 from the master sheet, with real guardian/phone/unit
    expect(summary.students.imported).toBe(3)
    const ahmed = db.prepare('SELECT * FROM students WHERE name = ?').get('أحمد محمد')
    ...
    const janAhmed = db.prepare(
      'SELECT * FROM payments WHERE student_id = ? AND month = ? AND year = ?'
    ).get(ahmed.id, 'يناير', 2025)
    expect(janAhmed.status).toBe('paid')       // recomputed: paid >= total
    expect(janAhmed.total).toBe(3500)

    const fatima = db.prepare('SELECT id FROM students WHERE name = ?').get('فاطمة علي')
    const janFatima = db.prepare(
      'SELECT status, balance FROM payments WHERE student_id = ? AND month = ?'
    ).get(fatima.id, 'يناير')
  ...
  it('is idempotent and resilient on re-import (US1)', async () => {
    ...
    expect(summary.students.imported).toBe(0)
    ...
    expect(db.prepare('SELECT COUNT(*) c FROM students').get().c).toBe(before.students)
    ...
    // Blank-name row never created a student and never threw
