...
describe('A student with more than one teacher gets a separate attendance/payment row per teacher', () => {
  let db: any
  let ahmedId: number
  let saraId: number
  ...
  beforeAll(() => {
    ...
    // Two service enrollments for the same student, each with a different teacher.
    db.prepare(`
      INSERT INTO student_services (student_id, service, unit, price, teacher_id, created_at, updated_at)
      VALUES (?, 'Speech Therapy', 'جلسة', 100, ?, ?, ?)
    `).run(studentId, ahmedId, now, now)
    db.prepare(`
      INSERT INTO student_services (student_id, service, unit, price, teacher_id, created_at, updated_at)
      VALUES (?, 'Occupational Therapy', 'جلسة', 100, ?, ?, ?)
    `).run(studentId, saraId, now, now)
  ...
  it('attendance:getSheet shows two rows for the student — one per teacher', async () => {
    const sheet = await getSheet(null, { session_id: sessionId })
    const studentRows = sheet.filter((r: any) => r.student_id === studentId)
    expect(studentRows.length).toBe(2)
    const teacherIds = studentRows.map((r: any) => r.teacher_id).sort()
  ...
  it('recording attendance for one teacher does not affect the other teacher\'s row', async () => {
    await record(null, { session_id: sessionId, records: [{ student_id: studentId, teacher_id: ahmedId, status: 'attended', teacher_status: 'present' }] })

    const sheet = await getSheet(null, { session_id: sessionId })
    const ahmedRow = sheet.find((r: any) => r.student_id === studentId && r.teacher_id === ahmedId)
    const saraRow = sheet.find((r: any) => r.student_id === studentId && r.teacher_id === saraId)
  ...
  it('recording attendance for both teachers generates two independent payments', async () => {
    await record(null, { session_id: sessionId, records: [{ student_id: studentId, teacher_id: saraId, status: 'attended', teacher_status: 'present' }] })

    const ahmedPayment = db.prepare('SELECT * FROM teacher_payments WHERE teacher_id = ? AND student_id = ?').get(ahmedId, studentId) as any
    const saraPayment = db.prepare('SELECT * FROM teacher_payments WHERE teacher_id = ? AND student_id = ?').get(saraId, studentId) as any
  ...
  it('voiding one teacher\'s payment (via a disqualifying edit) does not touch the other teacher\'s payment', async () => {
    await record(null, { session_id: sessionId, records: [{ student_id: studentId, teacher_id: ahmedId, status: 'absent_excused', teacher_status: 'present' }] })

    const ahmedPayment = db.prepare('SELECT status FROM teacher_payments WHERE teacher_id = ? AND student_id = ?').get(ahmedId, studentId) as any
    const saraPayment = db.prepare('SELECT status FROM teacher_payments WHERE teacher_id = ? AND student_id = ?').get(saraId, studentId) as any
