...
describe('Payroll Report CSV export (feature 007, FR-002/FR-005/FR-009) — row-shaping and totals', () => {
  ...
  beforeAll(() => {
    ...
    `).run(sessionId, studentId, now, now, teacherId).lastInsertRowid)
    ...
    `).run(teacherId, studentId, attendanceRecordId, now, now)
...
describe('Student Report CSV export (feature 007, US3/FR-007) — attendance percentage computation', () => {
  let db: any
  let teacherId: number
  let tmpFile: string
  ...
  beforeAll(() => {
    ...
    tmpFile = path.join(os.tmpdir(), `student-report-test-${Date.now()}.csv`)
  })
  ...
  function addSessionAttendance(studentId: number, date: string, status: string) {
    const now = new Date().toISOString()
    const sessionId = Number(db.prepare(
    ...
    db.prepare(`
      INSERT INTO attendance_records (session_id, student_id, status, recorded_at, updated_at, attended_teacher_id, teacher_status)
      VALUES (?, ?, ?, ?, ?, ?, 'present')
    `).run(sessionId, studentId, status, now, now, teacherId)
  }
  ...
  it('computes attendance percentage as attended / total sessions', async () => {
    ...
    addSessionAttendance(studentId, '2026-07-01', 'attended')
    addSessionAttendance(studentId, '2026-07-02', 'attended')
    addSessionAttendance(studentId, '2026-07-03', 'absent_unexcused')
    addSessionAttendance(studentId, '2026-07-04', 'absent_excused')

    await buildCsvFile('studentReport', { studentId, lang: 'en' }, tmpFile)
    const content = fs.readFileSync(tmpFile, 'utf8').replace(/^﻿/, '')
    // 2 attended / 4 total = 50%
    expect(content).toContain('Attendance Percentage,50%')
  })

  it('renders attendance percentage as N/A for a student with no attendance yet, not an error', async () => {
    const now = new Date().toISOString()
    const studentId = Number(db.prepare(`
      INSERT INTO students (name, guardian, guardian_phone, service, unit, price, reg_date, created_at, updated_at)
      VALUES ('NewStudent', 'Guardian', '0100', 'جلسة', 'جلسة', 100, '2026-01-01', ?, ?)
    `).run(now, now).lastInsertRowid)
