...
ipcMain.handle('salaryTypes:add', async (_event, input) => {
  try {
    ...
    const validModes = ['fixed_monthly', 'per_session_fixed', 'per_session_pct', 'hybrid', 'per_student_session', 'hourly']
    ...
    // Hourly pay is meaningless without a unit price for the hour — the timer has nothing to
    // multiply by, so the employee would silently earn nothing.
    if (mode === 'hourly' && (hourly_rate == null || Number(hourly_rate) <= 0)) {
      throw new Error('سعر الساعة مطلوب ويجب أن يكون أكبر من صفر / Hourly rate is required and must be greater than zero')
    }
...
ipcMain.handle('salaryTypes:update', async (_event, { id, patch }) => {
  try {
    ...
    const hourly_rate = patch.hourly_rate !== undefined ? patch.hourly_rate : st.hourly_rate
    if (mode === 'hourly' && (hourly_rate == null || Number(hourly_rate) <= 0)) {
      throw new Error('سعر الساعة مطلوب ويجب أن يكون أكبر من صفر / Hourly rate is required and must be greater than zero')
    }
    db.prepare(`
      UPDATE salary_types SET name = ?, mode = ?, monthly_rate = ?, session_rate = ?, session_pct = ?, hourly_rate = ?, updated_at = ?, synced = 0 WHERE id = ?
    `).run(name, mode, monthly_rate, session_rate, session_pct, hourly_rate, new Date().toISOString(), id)
